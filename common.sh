...
if [[ -d "${srcdir}/modules/luci-mod-system" ]]; then
  THEME_BRANCH="Theme2"
  rm -rf "${srcdir}"
  gitsvn https://github.com/bottle05/luci-theme-argon/tree/master "${HOME_PATH}/package/luci-theme-argon"
else
  THEME_BRANCH="Theme1"
  rm -rf "${srcdir}"
  gitsvn https://github.com/bottle05/luci-theme-argon/tree/18.06 "${HOME_PATH}/package/luci-theme-argon"
fi

echo "src-git danshui https://github.com/bottle05/openwrt-package.git;$SOURCE" >> "${HOME_PATH}/feeds.conf.default"
...
if [[ -z "$(find "$HOME_PATH/package" -type d -name "default-settings" -print)" ]] && [[ "${THEME_BRANCH}" == "Theme2" ]]; then
  gitsvn https://github.com/bottle05/common/tree/main/Share/default-settings "${HOME_PATH}/package/default-settings"
  ...
  gitsvn https://github.com/bottle05/common/tree/main/Share/default-setting "${HOME_PATH}/package/default-settings"
  grep -qw "libustream-wolfssl" "${HOME_PATH}/include/target.mk" && sed -i 's?\<libustream-wolfssl\>?libustream-openssl?g' "${HOME_PATH}/include/target.mk"
  ! grep -qw "dnsmasq-full" "${HOME_PATH}/include/target.mk" && sed -i 's?\<dnsmasq\>?dnsmasq-full?g' "${HOME_PATH}/include/target.mk"
  ! grep -qw "default-settings" "${HOME_PATH}/include/target.mk" && sed -i 's?DEFAULT_PACKAGES:=?DEFAULT_PACKAGES:=default-settings?g' "${HOME_PATH}/include/target.mk"
fi
...
if [[ ! -d "${HOME_PATH}/feeds/packages/devel/packr" ]]; then
  gitsvn https://github.com/bottle05/common/tree/main/Share/packr ${HOME_PATH}/feeds/packages/devel/packr
fi
